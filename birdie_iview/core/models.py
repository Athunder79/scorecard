...
class Shot(models.Model):
    ...
    shot_lie = models.CharField(max_length=100, null=True, blank=True)
    shot_result = models.CharField(max_length=100, null=True, blank=True)
    ...
    def get_shot_lie_list(self):
        """Return miss types as a list, e.g. ['rough', 'bunker']"""
        if self.shot_lie:
            return [m.strip() for m in self.shot_lie.split(",") if m.strip()]
        return []
    
    def get_shot_results_list(self):
        """Return miss types as a list, e.g. ['long', 'right']"""
        if self.shot_result:
            return [m.strip() for m in self.shot_result.split(",") if m.strip()]
        return []   
